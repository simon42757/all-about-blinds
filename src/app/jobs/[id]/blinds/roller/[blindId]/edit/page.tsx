  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({
        ...
        quantity: 2,
        cost: 245.50,
        aoi: 'Cream color, chain on right. Blackout fabric with UV protection. Safe by design with child safety features.'
      });
    }, 300);
  });
};
...
export default function EditRollerBlind() {
  ...
  const [blind, setBlind] = useState<RollerBlind | null>(null);
  ...
  const handleSubmit = async (values: RollerBlind) => {
    setSubmitting(true);
    try {
      ...
      router.push(`/jobs/${jobId}/blinds/roller/${blindId}`);
    } catch (error) {
  ...
  return (
    <main className="container mx-auto px-4 py-6 max-w-md">
      <header className="flex items-center mb-6">
        ...
        <h1 className="text-xl font-bold text-white">Edit Roller Blind</h1>
      </header>
            ...
            <Form className="space-y-4">
              ...
              <div>
                ...
                <Field 
                  ...
                  rows="3"
                  className="input-field" 
                  placeholder="Color, fabric type, chain position, etc."
                />
